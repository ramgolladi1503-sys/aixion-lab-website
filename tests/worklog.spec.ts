...
test("Home stays cinematic and directs operational activity to Pulse", async ({ page }) => {
  await page.goto("/", { waitUntil: "networkidle" });
  await expect(page.locator(".pulse-preview")).toHaveCount(0);
  await expect(page.getByRole("link", { name: /Open Pulse/i })).toBeVisible();
  await expect(page.locator(".sharp-system-row")).toHaveCount(4);
  await expect(page.locator(".home-worklog-strip")).toHaveCount(0);
});
